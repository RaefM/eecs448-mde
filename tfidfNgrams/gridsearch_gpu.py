...
from cuml.metrics import balanced_accuracy_score
import joblib
...
if __name__ == "__main__":
    ...
    print("Reading base...")
    base_df = cd.read_csv('aita_preprocessed.csv', sep='\t')
    X = base_df.processed_body.values

    # feel free to comment this out if using Albert's csvs
    # if so, use uni_df = cd.read_csv(<unigram csv>, sep='\t') (assuming tab sep)
    # Start block
    ...
    uni_df = cd.DataFrame.from_gpu_matrix(X_uni)
    bi_df = cd.DataFrame.from_gpu_matrix(X_bi)
    tri_df = cd.DataFrame.from_gpu_matrix(X_tri)
    # End block

    print("Reading doc2vec...")
    doc2vec_df = cd.read_csv('aita_doc2vec.csv', sep='\t')
