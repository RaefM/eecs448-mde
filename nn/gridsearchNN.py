...
Saves best model it finds to best_<model_type>_nn.pt
"""
...
def main():
    ...
    for cnn_hd, rnn_hd, dr, lr, wd in tqdm(
        ...
        if stats['accuracy'] > best_accuracy:
            ...
            torch.save(best_model.state_dict(), 'best_' + model_type + '_nn.pt')
    ...
    print(f"UAR on test set: {test_uar}, Accuracy on test set: {test_accuracy} with loss {test_total_loss}")
